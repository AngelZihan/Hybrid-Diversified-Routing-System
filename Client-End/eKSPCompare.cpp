...
int Graph::eKSPCompare(int ID1, int ID2, int k, vector<int>& kResults, vector<vector<int> >& vkPath, double t, int& countNumber, int& popPath)
{
...
//	cout << "finish Arc" << endl;
   ...
   /* unordered_map<int, int> mPos;
    ...
    if(!bCountNumber){
        ...
        cout << "eKSPCompare countNumber: "<< countNumber << " Pop Path: " << popPath << endl;
    }
    //t2 = std::chrono::high_resolution_clock::now();
    //time_span = std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1);
