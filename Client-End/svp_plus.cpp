...
vector<Path> svp_plus(RoadNetwork *rN, NodeID source, NodeID target, unsigned int k, double theta) {
	...
	while(resPathsFinal.size()<k && !svpQueue.empty()) {
		...
		for(unsigned int k=0;k<resPathsFinal.size();k++) {
            sim = tempP.overlap_ratio(rN,resPathsFinal[k]);
			if(sim > theta) {
				check = false;
				break;
			}
...
pair<vector<Path>,double> svp_plus_complete(RoadNetwork *rN, NodeID source, NodeID target, unsigned int k, double theta, float& AveSim, float& minSim, float& maxSim) {
	AveSim = 0;
	minSim = 1;
	maxSim = 0;
    int count11 = 0;
	...
	while(resPathsFinal.size()<k && !svpQueue.empty()) {
        ...
        int count1 = 0;
		for(unsigned int k=0;k<resPathsFinal.size();k++) {
			currentTheta = tempP.overlap_ratio(rN,resPathsFinal[k]);
	...
	if(resPathsFinal.size() < k) {
		...
		if(candidatePaths.size() < k) {
			float AveSim1 = 0,minSim = 0,maxSim = 0;
			vector<Path> ksp = onepass(rN,source,target,k,0.999,AveSim1,minSim,maxSim); // Can be replaced with a better k-shortest path algorithm.
	...
	//AveSim = AveSim / (resPathsFinal.size()*(resPathsFinal.size() - 1)/2);
