...
Path astar_limited(RoadNetwork *rN, NodeID source, NodeID target, vector<int> &bounds, unordered_set<EdgeNew,boost::hash<EdgeNew>> &deletedEdges) {
    ...
    while (!queue.empty()) {
        ...
        if (curLabel->node_id == target) { // Destination has been found
            ...
            for (iterAdj = rN->adjListOut[curLabel->node_id].begin(); iterAdj != rN->adjListOut[curLabel->node_id].end(); iterAdj++) {
                ...
                EdgeNew e(make_pair(curLabel->node_id,iterAdj->first));
