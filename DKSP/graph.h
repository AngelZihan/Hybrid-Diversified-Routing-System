...
public:
	...
	Graph(string filename)
	{
        ...
        //else if(filename == "DFSTest")
        else if(filename == "./ExampleGraph2")
		{
			readExampleMap(filename);
		}
		/*else if(filename == "/media/TraminerData/s4606021/Project/USA-road-d.NY.co")
		else if(filename == "/Users/angel/CLionProjects/Project2/USA-road-d.NY.co")
        {
            readTestCoor(filename);
        }*/
		else
		{
			readUSMap(filename);
			//readUSMapCost(filename);
			//readTestMap(filename);
		}
	}
...
//	int DijkstraSideCostPruneCSP(int ID1, int ID2, vector<vector<pair<int, int> > >& adjSideDist, vector<vector<pair<int, int> > >& adjSideRoad, vector<int>& vSPTParent, vector<int>& vSPTParentEdge, vector<int>& vSPTDistance, vector<label>& vLabel, vector<int>& vIgnoredNode, vector<int>& vPath, vector<int>& vPathEdge, bool& bE);
    ...
    int EdgesBlocking(int ID1, int ID2, int k, vector<int>& kResults, vector<vector<int> >& vkPath, double t, int& countNumber, int& popPath, float& AveSim, float& minSim, float& maxSim);
